...
async function scrapeProduct(url) {
  ...
  try {
    ...
    if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
      // Running on Vercel/Lambda - use dynamic import with proper configuration
      const chromiumPkg = await import('@sparticuz/chromium');
      const chromiumBinary = chromiumPkg.default || chromiumPkg;
      
      // Configure chromium with proper settings
      await chromiumBinary.font('https://raw.githack.com/googlei18n/noto-emoji/master/fonts/NotoColorEmoji.ttf');
      
      // Get executable path with proper configuration
      const executablePath = await chromiumBinary.executablePath({
        forceDownload: true, // Force download if not present
      });
