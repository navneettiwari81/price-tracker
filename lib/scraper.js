...
async function scrapeProduct(url) {
  ...
  try {
    ...
    if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
      ...
      let chromiumBinary;
      try {
        // Import chromium with multiple fallback strategies
        const chromiumImport = await import('@sparticuz/chromium');
        
        // Handle different export patterns
        if (chromiumImport.default) {
          chromiumBinary = chromiumImport.default;
        } else if (chromiumImport.chromium) {
          chromiumBinary = chromiumImport.chromium;
        } else {
          chromiumBinary = chromiumImport;
        }
        
        console.log('Chromium binary type:', typeof chromiumBinary);
        console.log('Available methods:', Object.keys(chromiumBinary || {}));
        
        // Verify required methods exist
        if (!chromiumBinary?.executablePath) {
          throw new Error('executablePath method not found on chromium binary');
        }
        
        // Get executable path - try without parameters first
        let executablePath;
        try {
          executablePath = await chromiumBinary.executablePath();
        } catch (pathError) {
          console.warn('First executablePath attempt failed:', pathError.message);
        ...
        // Try without @sparticuz/chromium as fallback
        console.log('Attempting fallback without @sparticuz/chromium');
        browserConfig = {
          headless: true,
          args: [
            '--no-sandbox',
